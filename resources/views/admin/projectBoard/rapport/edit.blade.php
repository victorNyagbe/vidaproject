      ...
      <div class="row">
          <div class="col-12 pb-5">
              ...
              <form action="{{ route('admin.projectBoard.rapport.destroy', $rapport) }}" method="get" >
                @csrf
                <button type="submit" class="btn btn-danger send-link" onclick="return confirm('Êtes-vous certain de vouloir supprimer ce rapport ? Cette action est irréversible.');"><i class="bi bi-trash-fill"></i> Supprimer</button>
              </form>
              <form action="{{ route('admin.projectBoard.rapport.downloadPdf', $rapport) }}" method="post" target="_blank">
                @csrf
                <button type="submit" class="btn btn-primary show-btn"><i class="bi bi-file-earmark-arrow-down"></i> Télécharger</button>
              </form>
